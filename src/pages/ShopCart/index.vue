  ...
  <div class="cart">
    ...
    <div class="cart-main">
      ...
      <div class="cart-body">
        <ul
          class="cart-list"
          v-for="(cartInfo, index) in cartListInfo"
          :key="cartInfo.id"
        >
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input
          class="chooseAll"
          type="checkbox"
          :checked="isAllCheck&&cartListInfo.length>0"
          @click="selectAllShopCart($event)"
        />
        <span>全选</span>
      </div>
      <div class="option">
...
export default {
  ...
  methods: {
    ...
    //修改选中状态
    async checkCart(cartInfo, event) {
      try {
    ...
    //全选
    async selectAllShopCart(event) {
      let checked = event.target.checked ? "1" : "0";
      try {
        await this.$store.dispatch("selectAllShopCart", checked);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
